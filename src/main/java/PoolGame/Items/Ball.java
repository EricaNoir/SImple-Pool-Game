...
public class Ball implements Drawable, Movable {
    ...
    public Ball(String colour, double xPos, double yPos, double xVel, double yVel, double mass, BallType type, BallPocketStrategy pocketAction) throws IllegalArgumentException {
        ...
        this.visualCue = new Group();
        this.mouseDragLine = new Line();
        this.stick = new Line();
        this.stickHead = new Circle();
        visualCue.getChildren().add(mouseDragLine);
        visualCue.getChildren().add(stick);
        visualCue.getChildren().add(stickHead);
        this.visualCue.setVisible(false);
    ...
    public void addToGroup(ObservableList<Node> groupChildren) {
        ...
        groupChildren.add(this.visualCue);
    ...
    private void registerCueBallMouseAction() {
        this.shape.setOnMouseDragged(
            (actionEvent) -> {
                if (this.hasStopped()) {
                    this.visualCue.setVisible(true);
                    // hint line
                    mouseDragLine.getStrokeDashArray().add(5.0);
                    mouseDragLine.setStrokeDashOffset(10);
                    ...
                    // stick head
                    this.stickHead.setFill(Color.WHITE);
                    this.stickHead.setRadius(6);
                    this.stickHead.setCenterX(actionEvent.getSceneX());
                    this.stickHead.setCenterY(actionEvent.getSceneY());

                    // stick
                    this.stick.setStrokeWidth(8);
                    this.stick.setStroke(Color.GREY);
                    this.stick.setStartX(actionEvent.getSceneX());
                    this.stick.setStartY(actionEvent.getSceneY());

                    // calculate end position of the stick
                    Double x1 = this.shape.getCenterX();
                    Double y1 = this.shape.getCenterY();
                    Double x2 = actionEvent.getSceneX();
                    Double y2 = actionEvent.getSceneY();
                    Double theta = Math.atan((y2 - y1) / (x2 - x1));

                    Double x3;
                    Double y3;
                    if ((x2 - x1) > 0) {
                        x3 = x2 + 150 * Math.cos(theta);
                        y3 = y2 + 150 * Math.sin(theta);
                    }
                    else {
                        x3 = x2 - 150 * Math.cos(theta);
                        y3 = y2 - 150 * Math.sin(theta);
                    ...
                    this.stick.setEndX(x3);
                    this.stick.setEndY(y3);
    ...
    */
    ...
    public void fallIntoPocket(Game game) {
        ...
        // When balls falls into pocket, gain crossbarring scores
        ScoreBoard scoreBoard = ScoreBoard.getScoreBoard();
        scoreBoard.gainScore(this);
    }
}
